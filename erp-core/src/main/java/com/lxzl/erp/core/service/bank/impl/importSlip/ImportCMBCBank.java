...
public class ImportCMBCBank {
     ...
     */
    public ServiceResult<String, BankSlipDO> saveCMBCBank(BankSlip bankSlip, InputStream inputStream) throws Exception {
    ...
    public ServiceResult<String, List<BankSlipDetailDO>> getCMBCBankData(Sheet sheet, Row row, Cell cell, BankSlipDO bankSlipDO, Date now) throws Exception {
        ...
        for (int j = sheet.getFirstRowNum(); j <= sheet.getLastRowNum(); j++) {
            ...
            if (row != null) {
                ...
                // todo 以下可以直接存数据
                String payerName = null;  //收/付方名称
                String tradeTime = null;  //交易日
                ...
                if (j > next) {
                    ...
                    payerName = (row.getCell(payerNameNo) == null ? "" : getValue(row.getCell(payerNameNo)).replaceAll("\\s+", ""));  //收/付方名称
                    tradeTime = (row.getCell(payTimeNo) == null ? "" : getValue(row.getCell(payTimeNo)).replaceAll("\\s+", ""));  //交易日
