...
//                }
 ...
 @Override
    ...
    public ServiceResult<String, String> importK3HistoricalRefundList(K3ReturnOrderQueryParam k3ReturnOrderQueryParam) {
        ...
        // 保存k3数据列表
        saveBillDatas(billDatas);
        importResult.setResult(JSONObject.toJSONString(billDatas));
        return importResult;
    }
    ...
    /** 保存k3历史退货订单数据 */
    private int saveBillDatas(List<K3HistoricalReturnOrder> billDatas) {
        // 数据过滤
        List<K3HistoricalReturnOrder> needSaveBillDatas = filterNotNeedSaveBillDatas(billDatas);
        if (logger.isInfoEnabled()) {
        ...
        // 保存退货订单数据
        List<K3ReturnOrderDO> k3ReturnOrderDOS = saveK3ReturnOrders(billDatas);
        ...
        // 保存退货订单详情数据
        saveK3ReturnOrderDetails(billDatas);
        return needSaveBillDatas.size();
    }

    /** 过滤不需要保存的退货单信息 */
    private List<K3HistoricalReturnOrder> filterNotNeedSaveBillDatas(List<K3HistoricalReturnOrder> billDatas) {
    ...
    private List<K3HistoricalReturnOrder> getNeedAddK3ReturnBills(List<K3HistoricalReturnOrder> billDatas, List<K3ReturnOrderDO> k3ReturnOrderDOS) {
        ...
        for (K3HistoricalReturnOrder billData : billDatas) {
            ...
            if (billData.getK3ReturnOrder().getReturnReasonType() == null) {
                logger.error("=====================k3ReturnOrder:returnReasonType的字段为空================");
                needAddFlag = false;
            }
            if (billData.getK3ReturnOrder().getDeliverySubCompanyId() == null) {
                logger.error("=====================k3ReturnOrder:deliverySubCompanyId的字段为空================");
                needAddFlag = false;
            }
            if (needAddFlag) {
