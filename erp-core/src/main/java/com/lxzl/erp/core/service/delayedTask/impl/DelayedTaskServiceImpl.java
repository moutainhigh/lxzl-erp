package com.lxzl.erp.core.service.delayedTask.impl;
...
import com.lxzl.erp.dataaccess.domain.delayedTask.DelayedTaskConfigExportStatementDO;
...
public class DelayedTaskServiceImpl implements DelayedTaskService{
    ...
    private ExecutorService threadPoolTaskExecutor = Executors.newFixedThreadPool(10, new ThreadFactoryDefault("rewar"));
     ...
     */
    @Override
    public ServiceResult<String, Page<DelayedTask>> pageDelayedTask(DelayedTaskQueryParam delayedTaskQueryParam) {
        ServiceResult<String, Page<DelayedTask>> serviceResult = new ServiceResult<>();

        PageQuery pageQuery = new PageQuery(delayedTaskQueryParam.getPageNo(), delayedTaskQueryParam.getPageSize());
        Map<String, Object> maps = new HashMap<>();
        maps.put("start", pageQuery.getStart());
        maps.put("pageSize", pageQuery.getPageSize());
        if (!userSupport.isSuperUser()) {
            delayedTaskQueryParam.setCreateUser(userSupport.getCurrentUserId().toString());
        }
        maps.put("delayedTaskQueryParam", delayedTaskQueryParam);
//        maps.put("permissionParam", permissionSupport.getPermissionParam(PermissionType.PERMISSION_TYPE_USER));
    ...
    public ServiceResult<String, DelayedTask> getCustomerStatementDelayedTask(StatementOrderMonthQueryParam statementOrderMonthQueryParam,ServiceResult<String, DelayedTask> result,DelayedTask delayedTask) throws Exception {
        ...
        Date date = new Date();
        if (delayedTaskConfigExportStatementDO != null) {
            // TODO: 2018\7\27 0027  开始线程导数据
            // TODO: 2018\7\27 0027 判断该请求用户是否在排队中或者处理中有，如果有完成
            DelayedTaskDO newdelayedTaskDO = ConverterUtil.convert(delayedTask, DelayedTaskDO.class);
            List<DelayedTaskDO> delayedTaskDOByUserIdList = delayedTaskMapper.findByUserId(userSupport.getCurrentUserId().toString());
            //已经存在排队中或者处理中
            if (CollectionUtil.isNotEmpty(delayedTaskDOByUserIdList)) {
                result.setErrorCode(ErrorCode.TASK_REQUEST_HAVE_FOR_THIS_USER_ID);//提示，您已经有排队中或处理中的导出任务，请该任务完成后再进行导出
                return result;
            } else {
                //获取等待中的最后一个和处理中的个数
                DelayedTaskDO delayedTaskDOQueueLast = delayedTaskMapper.findQueueLast();
                Integer processingCount = delayedTaskMapper.findProcessingCount();
                if (delayedTaskDOQueueLast != null) {//有处于等待中的，该记录也存为排队中
                    newdelayedTaskDO.setTaskStatus(DelayedTaskStatus.DELAYED_TASK_QUEUE);//排队中
                    newdelayedTaskDO.setQueueNumber(delayedTaskDOQueueLast.getQueueNumber()+1);
                    newdelayedTaskDO.setThreadName(null);
                    newdelayedTaskDO.setProgressRate(0.0000);
                    newdelayedTaskDO.setFileUrl(null);
                    newdelayedTaskDO.setDataStatus(CommonConstant.COMMON_CONSTANT_YES);
                    newdelayedTaskDO.setCreateTime(date);
                    newdelayedTaskDO.setUpdateTime(date);
                ...
                } else if (processingCount == 10 ) {
                    newdelayedTaskDO.setTaskStatus(DelayedTaskStatus.DELAYED_TASK_QUEUE);//排队中
                    newdelayedTaskDO.setQueueNumber(1);
                    newdelayedTaskDO.setThreadName(null);
                    newdelayedTaskDO.setProgressRate(0.0000);
                    newdelayedTaskDO.setFileUrl(null);
                    newdelayedTaskDO.setDataStatus(CommonConstant.COMMON_CONSTANT_YES);
                    ...
                    newdelayedTaskDO.setQueueNumber(CommonConstant.COMMON_ZERO);
                    newdelayedTaskDO.setThreadName(null);
                    newdelayedTaskDO.setProgressRate(0.0000);
                    newdelayedTaskDO.setFileUrl(null);
                    newdelayedTaskDO.setDataStatus(CommonConstant.COMMON_CONSTANT_YES);
                ...
                //调用线程导出对账单
                exportCustomerStatementAsynchronous(newdelayedTaskDO, date,statementOrderMonthQueryParam,getSession());
            }
        }else {
            // TODO: 2018\7\27 0027 进程导数据
            DelayedTaskDO delayedTaskDO = ConverterUtil.convert(delayedTask, DelayedTaskDO.class);
            try {
                //初始化任务列表
                delayedTaskDO.setTaskStatus(DelayedTaskStatus.DELAYED_TASK_QUEUE);//排队中
    ...
    //            System.out.println(sb.toString());
                ...
                for(int i = 1 ; i <= listCount ; i++){
                    ...
                    SetCheckStatementOrder setCheckStatementOrder = new SetCheckStatementOrder(hssfWorkbook, customerName, statementOrderStartTime, beforePeriodUnpaid, allPeriodUnpaid, totalEverPeriodAmountMap, totalEverPeriodPaidAmountMap, previousSheetName, simpleDateFormat, checkStatementOrder).invoke();
                ...
                try {
                    outputExcel(fileName,hssfWorkbook);
                } catch (Exception e) {
                    e.printStackTrace();
...
//            hssfWorkbook.write(o);
     ...
     * @param fileName 文件名
     */
    public void outputExcel(String fileName,XSSFWorkbook hssfWorkbook) throws Exception{
    ...
    //处理对账单业务逻辑
    private class ProcesscCustomerStatement {
        private boolean myResult;
        private StatementOrderMonthQueryParam statementOrderMonthQueryParam;
        private ServiceResult<String, DelayedTask> result;
        private List<CheckStatementOrder> checkStatementOrderList;
        private SimpleDateFormat simpleDateFormat;

        public ProcesscCustomerStatement(StatementOrderMonthQueryParam statementOrderMonthQueryParam, ServiceResult<String, DelayedTask> result, List<CheckStatementOrder> checkStatementOrderList) {
            this.statementOrderMonthQueryParam = statementOrderMonthQueryParam;
            this.result = result;
            this.checkStatementOrderList = checkStatementOrderList;
        }
        ...
        //先付后用找月份和结算开始时间在同一月的那一期
        public ProcesscCustomerStatement invoke() throws ParseException {
            Map<String,CheckStatementOrderDetail> bigDecimalMap = new HashMap<>();
    ...
    private class SetCheckStatementOrder {
        ...
        public SetCheckStatementOrder(XSSFWorkbook hssfWorkbook, String customerName, Date statementOrderStartTime, BigDecimal beforePeriodUnpaid, BigDecimal allPeriodUnpaid, Map<String, BigDecimal> totalEverPeriodAmountMap, Map<String, BigDecimal> totalEverPeriodPaidAmountMap, String previousSheetName, SimpleDateFormat simpleDateFormat, CheckStatementOrder checkStatementOrder) {
            ...
            this.checkStatementOrder = checkStatementOrder;
        }
