...
import com.lxzl.erp.core.service.statement.impl.support.StatementCommonSupport;
...
//            }
    ...
    private List<StatementOrderDetailDO> getStatementReletOrderDetailDOS(ReletOrderDO reletOrderDO, Date currentTime, Date rentStartTime, Integer loginUserId) {
        ...
        //当前续租是否进行结算单对齐(有分段结算时 不进行对齐)
        boolean isNeedAlign = true;
        ...
        if (isStatementDateChangeInOrderRentTime) {
            ...
            addStatementOrderDetailDOList = generateReletStatementDetailList(reletOrderDO, currentTime, statementDays, loginUserId, isNeedAlign);
        }
        return addStatementOrderDetailDOList;
    }
    ...
    ServiceResult<String, BigDecimal> reCreateOrderStatement(OrderDO orderDO, Integer statementDate, boolean clearStatementDateSplitCfg,boolean allowConfirmCustomer) {
        ...
        if (CollectionUtil.isNotEmpty(reletOrderDOList)) {
            for (ReletOrderDO reletOrderDO : reletOrderDOList) {
                ...
                //百分比支付方式续租单需修改状态
                reletOrderSupport.updateReletOrderPayModeOfBeforePercent(reletOrderDO);
                //旧结算信息已在前面清除这里不需再次处理
...
//        待发货、已发货、确认收货，部分归还，全部归还，完成 状态可重算
    ...
    private List<StatementOrderDetailDO> generateStatementDetailList(OrderDO orderDO, Date currentTime, Integer statementDays, Integer loginUserId) {
        ...
        if (CollectionUtil.isNotEmpty(orderDO.getOrderMaterialDOList())) {
            for (OrderMaterialDO orderMaterialDO : orderDO.getOrderMaterialDOList()) {
                ...
                List<StatementOrderDetailDO> statementOrderDetailDOList=generateOrderMaterialStatement(orderDO.getRentTimeLength(),orderDO.getStatementDate(), currentTime, statementDays, loginUserId, rentStartTime, buyerCustomerId, orderId, orderMaterialDO, itemAllAmount);
                if(CollectionUtil.isNotEmpty(statementOrderDetailDOList))addStatementOrderDetailDOList.addAll(statementOrderDetailDOList);
            }
        }
        ...
        if (BigDecimalUtil.compare(otherAmount, BigDecimal.ZERO) > 0) {
            // 其他费用统一结算
            StatementOrderDetailDO thisStatementOrderDetailDO = statementCommonSupport.buildStatementOrderDetailDO(buyerCustomerId, OrderType.ORDER_TYPE_ORDER, orderDO.getId(), OrderItemType.ORDER_ITEM_TYPE_OTHER, BigInteger.ZERO.intValue(), rentStartTime, rentStartTime, rentStartTime, BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO, otherAmount, currentTime, loginUserId, null);
    ...
    public List<StatementOrderDetailDO> generateOrderMaterialStatement(Integer rentTimeLength,Integer statementMode, Date currentTime, Integer statementDays, Integer loginUserId,  Date rentStartTime, Integer buyerCustomerId, Integer orderId, OrderMaterialDO orderMaterialDO, BigDecimal itemAllAmount) {
        List<StatementOrderDetailDO> addStatementOrderDetailDOList=new ArrayList<>();
        ...
        // 先确定订单需要结算几期
        Integer statementMonthCount = calculateStatementMonthCount(orderMaterialDO.getRentType(), rentTimeLength, orderMaterialDO.getPaymentCycle(), orderMaterialDO.getPayMode(), rentStartTimeCalendar.get(Calendar.DAY_OF_MONTH), statementDays);
        if (statementMonthCount == 1) {
            StatementOrderDetailDO statementOrderDetailDO = calculateOneStatementOrderDetail(orderMaterialDO.getRentType(), rentTimeLength, orderMaterialDO.getPayMode(), rentStartTime, itemAllAmount, buyerCustomerId, orderId, OrderItemType.ORDER_ITEM_TYPE_MATERIAL, orderMaterialDO.getId(), currentTime, loginUserId, null);
    ...
    void updateStatementOrderResult(StatementOrderDO statementOrderDO, BigDecimal rentAmount, BigDecimal rentDepositAmount, BigDecimal depositAmount, BigDecimal otherAmount, BigDecimal overdueAmount, Date currentTime, Integer loginUserId) {
        ...
        for (StatementOrderDetailDO statementOrderDetailDO : statementOrderDO.getStatementOrderDetailDOList()) {
            if (StatementOrderStatus.STATEMENT_ORDER_STATUS_INIT.equals(statementOrderDetailDO.getStatementDetailStatus())||StatementOrderStatus.STATEMENT_ORDER_STATUS_SETTLED_PART.equals(statementOrderDetailDO.getStatementDetailStatus())) {
                ...
                // 查询有没有冲正业务金额
                BigDecimal correctBusinessAmount = statementPaySupport.getStatementItemCorrectAmount(statementOrderDetailDO);
                ...
                if (BigDecimalUtil.compare(noPaidStatementDetailOverduePayAmount, BigDecimal.ZERO) > 0) {
                    ...
                    // 查询有没有冲正逾期金额
                    BigDecimal correctOverdueAmount = statementPaySupport.getStatementItemCorrectAmountByType(statementOrderDetailDO);
                ...
                if (OrderType.ORDER_TYPE_CHANGE.equals(statementOrderDetail.getOrderType())) {
                    if (OrderType.ORDER_TYPE_CHANGE.equals(statementOrderDetail.getOrderType())) {
                        ...
                        if (OrderItemType.ORDER_ITEM_TYPE_CHANGE_OTHER.equals(statementOrderDetail.getOrderItemType())) {
                            ...
                            key = statementOrderDetail.getOrderItemType() + "-" + statementOrderDetail.getOrderType() + "-" + statementOrderDetail.getOrderNo() + "-" + statementOrderDetail.getItemRentType() + "-" + statementOrderDetail.getReletOrderItemReferId();
                        }
                    }
                }
...
//        }
        ...
        if (CollectionUtil.isNotEmpty(k3ReturnOrderDO.getK3ReturnOrderDetailDOList())) {
            for (K3ReturnOrderDetailDO k3ReturnOrderDetailDO : k3ReturnOrderDO.getK3ReturnOrderDetailDOList()) {
                ...
                List<StatementOrderDetailDO> statementOrderDetailDOList = statementOrderDetailMapper.findByOrderItemTypeAndIdIngnoreOrderType(OrderItemType.ORDER_ITEM_TYPE_PRODUCT, orderProductDO.getId());
...
//                                }
                                ...
                                if (BigDecimalUtil.compare(BigDecimalUtil.sub(statementOrderDetailDO.getStatementDetailDepositPaidAmount(), statementOrderDetailDO.getStatementDetailDepositReturnAmount()), thisReturnDepositAmount) >= 0) {
                                    ...
                                    statementReturnSupport.saveStatementReturnRecord(statementOrderDO.getId(), statementOrderDO.getCustomerId(), statementOrderDetailDO.getOrderId(), statementOrderDetailDO.getOrderItemReferId(), StatementReturnType.RETURN_TYPE_DEPOSIT, thisReturnDepositAmount, returnTime, loginUserId, currentTime,OrderType.ORDER_TYPE_RETURN,k3ReturnOrderDetailDO.getReturnOrderId(),k3ReturnOrderDetailDO.getId());
                                }
                                if (BigDecimalUtil.compare(BigDecimalUtil.sub(statementOrderDetailDO.getStatementDetailRentDepositPaidAmount(), statementOrderDetailDO.getStatementDetailRentDepositReturnAmount()), thisReturnRentDepositAmount) >= 0) {
                                    ...
                                    statementReturnSupport.saveStatementReturnRecord(statementOrderDO.getId(), statementOrderDO.getCustomerId(), statementOrderDetailDO.getOrderId(), statementOrderDetailDO.getOrderItemReferId(), StatementReturnType.RETURN_TYPE_RENT_DEPOSIT, thisReturnRentDepositAmount, returnTime, loginUserId, currentTime,OrderType.ORDER_TYPE_RETURN,k3ReturnOrderDetailDO.getReturnOrderId(),k3ReturnOrderDetailDO.getId());
                                }
                                statementOrderDetailMapper.update(statementOrderDetailDO);
                                statementOrderMapper.update(statementOrderDO);
                            }
...
//                        payReturnAmount = BigDecimalUtil.sub(payReturnAmount, needPayAmount);
                        ...
                        if (addStatementOrderDetailDOMap.get(key) != null) {
                            ...
                            StatementOrderDetailDO thisStatementOrderDetailDO = statementCommonSupport.buildStatementOrderDetailDO(buyerCustomerId, OrderType.ORDER_TYPE_RETURN, k3ReturnOrderDO.getId(), OrderItemType.ORDER_ITEM_TYPE_RETURN_PRODUCT, k3ReturnOrderDetailDO.getId(), statementOrderDetailDO.getStatementExpectPayTime(), statementDetailStartTime, statementDetailEndTime, BigDecimalUtil.mul(payReturnAmount, new BigDecimal(-1)), BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO, currentTime, loginUserId, statementOrderDetailDO.getReletOrderItemReferId());
                            if (thisStatementOrderDetailDO != null) {
        ...
        if (CollectionUtil.isNotEmpty(k3ReturnOrderDO.getK3ReturnOrderDetailDOList())) {
            for (K3ReturnOrderDetailDO k3ReturnOrderDetailDO : k3ReturnOrderDO.getK3ReturnOrderDetailDOList()) {
                ...
                List<StatementOrderDetailDO> statementOrderDetailDOList = statementOrderDetailMapper.findByOrderItemTypeAndIdIngnoreOrderType(OrderItemType.ORDER_ITEM_TYPE_MATERIAL, orderMaterialDO.getId());
...
//                        payReturnAmount = BigDecimalUtil.sub(payReturnAmount, needPayAmount);
                        ...
                        if (addStatementOrderDetailDOMap.get(key) != null) {
                            ...
                            StatementOrderDetailDO thisStatementOrderDetailDO = statementCommonSupport.buildStatementOrderDetailDO(buyerCustomerId, OrderType.ORDER_TYPE_RETURN, k3ReturnOrderDO.getId(), OrderItemType.ORDER_ITEM_TYPE_RETURN_MATERIAL, k3ReturnOrderDetailDO.getId(), statementOrderDetailDO.getStatementExpectPayTime(), statementDetailStartTime, statementDetailEndTime, BigDecimalUtil.mul(payReturnAmount, new BigDecimal(-1)), BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO, currentTime, loginUserId, statementOrderDetailDO.getReletOrderItemReferId());
                            if (thisStatementOrderDetailDO != null) {
                                thisStatementOrderDetailDO.setStatementDetailType(StatementDetailType.STATEMENT_DETAIL_TYPE_OFFSET_RENT);
                                thisStatementOrderDetailDO.setReturnReferId(statementOrderDetailDO.getId());
                                addStatementOrderDetailDOMap.put(key, thisStatementOrderDetailDO);
        ...
        if (BigDecimalUtil.compare(otherAmount, BigDecimal.ZERO) > 0) {
            ...
            // 如果没有处理完差价，统一当天交钱
            StatementOrderDetailDO thisStatementOrderDetailDO = statementCommonSupport.buildStatementOrderDetailDO(buyerCustomerId, OrderType.ORDER_TYPE_RETURN, k3ReturnOrderDO.getId(), OrderItemType.ORDER_ITEM_TYPE_RETURN_OTHER, BigInteger.ZERO.intValue(), otherStatementTime, returnTime, returnTime, BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO, otherAmount, currentTime, loginUserId, null);
            if (thisStatementOrderDetailDO != null) {
                thisStatementOrderDetailDO.setStatementDetailType(StatementDetailType.STATEMENT_DETAIL_TYPE_OTHER);
                addStatementOrderDetailDOMap.put(key, thisStatementOrderDetailDO);
            }
 ...
 private boolean isReletOrderRefReturn(Date returnTime, ReletOrderDO reletOrder) {
    ...
    public ServiceResult<String, BigDecimal> createReturnOrderStatement(String returnOrderNo) {
                ...
                if (CollectionUtil.isNotEmpty(statementOrderDetailDOList)) {
                    for (int i = 0; i < statementOrderDetailDOList.size(); i++) {
                        ...
                        if (addStatementOrderDetailDOMap.get(key) != null) {
                            ...
                            StatementOrderDetailDO thisStatementOrderDetailDO = statementCommonSupport.buildStatementOrderDetailDO(buyerCustomerId, OrderType.ORDER_TYPE_RETURN, returnOrderMaterialBulkDO.getReturnOrderId(), OrderItemType.ORDER_ITEM_TYPE_RETURN_MATERIAL, returnOrderMaterialBulkDO.getReturnOrderMaterialId(), statementOrderDetailDO.getStatementExpectPayTime(), statementDetailStartTime, statementDetailEndTime, BigDecimalUtil.mul(payReturnAmount, new BigDecimal(-1)), BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO, currentTime, loginUser.getUserId(), null);
                            if (thisStatementOrderDetailDO != null) {
                                thisStatementOrderDetailDO.setStatementDetailType(StatementDetailType.STATEMENT_DETAIL_TYPE_OFFSET_RENT);
                                thisStatementOrderDetailDO.setReturnReferId(statementOrderDetailDO.getId());
                                addStatementOrderDetailDOMap.put(key, thisStatementOrderDetailDO);
        ...
        if (BigDecimalUtil.compare(otherAmount, BigDecimal.ZERO) > 0) {
            String key = OrderItemType.ORDER_ITEM_TYPE_RETURN_OTHER.toString();
            // 如果没有处理完差价，统一当天交钱
            StatementOrderDetailDO thisStatementOrderDetailDO = statementCommonSupport.buildStatementOrderDetailDO(buyerCustomerId, OrderType.ORDER_TYPE_RETURN, returnOrderDO.getId(), OrderItemType.ORDER_ITEM_TYPE_RETURN_OTHER, BigInteger.ZERO.intValue(), otherStatementTime, returnTime, returnTime, BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO, otherAmount, currentTime, loginUser.getUserId(), null);
            if (thisStatementOrderDetailDO != null) {
                thisStatementOrderDetailDO.setStatementDetailType(StatementDetailType.STATEMENT_DETAIL_TYPE_OTHER);
                addStatementOrderDetailDOMap.put(key, thisStatementOrderDetailDO);
            }
            ...
            for (K3ChangeOrderDetailDO k3ChangeOrderDetailDO : k3ChangeOrderDetailDOList) {
                ...
                if (nextStatement != null) {
                    ...
                    if (BigDecimalUtil.compare(thisMonthDiff, BigDecimal.ZERO) > 0) {
                        ...
                        StatementOrderDetailDO changeStatementOrderDetailDO = statementCommonSupport.buildStatementOrderDetailDO(nextStatement.getCustomerId(), OrderType.ORDER_TYPE_CHANGE,
                ...
                for (int i = 0; i < statementOrderDetailDOList.size(); i++) {
                    ...
                    StatementOrderDetailDO changeStatementOrderDetailDO = statementCommonSupport.buildStatementOrderDetailDO(statementOrderDetailDO.getCustomerId(), OrderType.ORDER_TYPE_CHANGE,
                            k3ChangeOrderDetailDO.getChangeOrderId(), orderItemType, Integer.parseInt(k3ChangeOrderDetailDO.getOrderItemId()),
    ...
    public ServiceResult<String, BigDecimal> createChangeOrderStatement(String changeOrderNo) {
        ...
        if (CollectionUtil.isNotEmpty(changeOrderProductEquipmentDOList)) {
            for (ChangeOrderProductEquipmentDO changeOrderProductEquipmentDO : changeOrderProductEquipmentDOList) {
                ...
                if (OrderRentType.RENT_TYPE_DAY.equals(orderProductDO.getRentType())) {
                    ...
                    StatementOrderDetailDO thisStatementOrderDetailDO = statementCommonSupport.buildStatementOrderDetailDO(buyerCustomerId, OrderType.ORDER_TYPE_CHANGE, changeOrderProductEquipmentDO.getChangeOrderId(), OrderItemType.ORDER_ITEM_TYPE_CHANGE_PRODUCT, changeOrderProductEquipmentDO.getChangeOrderProductId(), statementDetailStartTime, statementDetailStartTime, statementDetailEndTime, productEquipmentDiffAmount, BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO, currentTime, loginUser.getUserId(), null);
                    addStatementOrderDetailDOList.add(thisStatementOrderDetailDO);
                ...
                if (OrderRentType.RENT_TYPE_DAY.equals(orderMaterialDO.getRentType())) {
                    ...
                    if (CollectionUtil.isNotEmpty(statementOrderDetailDOList)) {
                        ...
                        for (int i = 0; i < statementOrderDetailDOList.size(); i++) {
                            ...
                            StatementOrderDetailDO thisStatementOrderDetailDO = statementCommonSupport.buildStatementOrderDetailDO(statementOrderDetailDO.getCustomerId(), OrderType.ORDER_TYPE_CHANGE, changeOrderMaterialBulkDO.getChangeOrderId(), OrderItemType.ORDER_ITEM_TYPE_CHANGE_MATERIAL, changeOrderMaterialBulkDO.getChangeOrderMaterialId(), statementOrderDetailDO.getStatementExpectPayTime(), statementDetailStartTime, statementDetailEndTime, payDiffAmount, BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO, currentTime, loginUser.getUserId(), null);
                            if (thisStatementOrderDetailDO != null) {
                                addStatementOrderDetailDOList.add(thisStatementOrderDetailDO);
                            }
                        }
        ...
        if (BigDecimalUtil.compare(otherAmount, BigDecimal.ZERO) > 0) {
            // 如果没有处理完差价，统一当天交钱
            StatementOrderDetailDO thisStatementOrderDetailDO = statementCommonSupport.buildStatementOrderDetailDO(buyerCustomerId, OrderType.ORDER_TYPE_RETURN, changeOrderDO.getId(), OrderItemType.ORDER_ITEM_TYPE_CHANGE_OTHER, BigInteger.ZERO.intValue(), currentTime, currentTime, currentTime, BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO, otherAmount, currentTime, loginUser.getUserId(), null);
            addStatementOrderDetailDOList.add(thisStatementOrderDetailDO);
        }
        saveStatementOrder(addStatementOrderDetailDOList, currentTime, loginUser.getUserId());
        result.setErrorCode(ErrorCode.SUCCESS);
...
//                    }
     ...
     *
     * @author ZhaoZiXuan
     * @date 2018/8/16 20:10
     * @param
     * @return
     */
    private Boolean isNeedAlignStatement(Integer orderId, Integer reletRentType){
        //当上一个单和当前续租都是按月时：需要对齐
        OrderDO orderDO = orderMapper.findByOrderId(orderId);
        Integer rentType = orderDO.getRentType();
        ReletOrderDO lastOfOrderReletOrderDO = reletOrderMapper.findRecentlyReletedOrderByOrderId(orderId);
        if (lastOfOrderReletOrderDO != null) {
            rentType = lastOfOrderReletOrderDO.getRentType();
        }
        Boolean isNeedAlign = false;
        if (OrderRentType.RENT_TYPE_MONTH.equals(rentType) && OrderRentType.RENT_TYPE_MONTH.equals(reletRentType)){
            isNeedAlign = true;
        }
        return isNeedAlign;
    }
    ...
    /**
     * 更新结算单支付日期（包括明细） 注意明细和资金转移
     *
     * @author ZhaoZiXuan
     * @date 2018/8/14 19:17
     * @param
     * @return
     */
    private void updateStatementOrderAndDetailExpectPayTime(Integer payMode, Integer orderId, Date statementExpectPayTime, Integer paymentCycle, Date currentTime, Integer loginUserId){
        if (OrderPayMode.PAY_MODE_PAY_AFTER.equals(payMode)){
            ...
            if (statementOrderDetailDODB != null){
                ...
                //若上一期刚好够整，则不进行结算单项转移
                Date thisPhaseDate = com.lxzl.se.common.util.date.DateUtil.monthInterval(statementOrderDetailDODB.getStatementStartTime(), paymentCycle);
                if (thisPhaseDate == null || thisPhaseDate.compareTo(statementOrderDetailDODB.getStatementExpectPayTime()) == 0){
                    return;
                }

                List<StatementOrderDetailDO> needOperateStatementOrderDetailDOList = new ArrayList<StatementOrderDetailDO>();
                Map<Integer, StatementOrderDetailDO> needOperateStatementOrderDetailDOMap = new HashMap<>();
                List<StatementOrderDetailDO> statementOrderDetailDOListDB = statementOrderDetailMapper.findByStatementOrderId(statementOrderDetailDODB.getStatementOrderId());
                //获取待修改的订单 结算单项
                for (StatementOrderDetailDO oneStatementOrderDetailDO : statementOrderDetailDOListDB){
                    if ((statementExpectPayTime != null && orderId != null)
                            && (statementExpectPayTime.compareTo(oneStatementOrderDetailDO.getStatementExpectPayTime()) != 0)
                            && orderId.equals(oneStatementOrderDetailDO.getOrderId())){
                        oneStatementOrderDetailDO.setStatementExpectPayTime(statementExpectPayTime);
                        needOperateStatementOrderDetailDOMap.put(oneStatementOrderDetailDO.getId(), oneStatementOrderDetailDO);
                    }
                }
                //获取待修改的退货单 结算单项
                for (StatementOrderDetailDO oneStatementOrderDetailDO : statementOrderDetailDOListDB){
                    if ((statementExpectPayTime != null && orderId != null && oneStatementOrderDetailDO.getReturnReferId() != null)
                            && OrderType.ORDER_TYPE_RETURN.equals(oneStatementOrderDetailDO.getOrderType())
                            && (statementExpectPayTime.compareTo(oneStatementOrderDetailDO.getStatementExpectPayTime()) != 0)
                            && needOperateStatementOrderDetailDOMap.containsKey(oneStatementOrderDetailDO.getReturnReferId())){
                        oneStatementOrderDetailDO.setStatementExpectPayTime(statementExpectPayTime);
                        needOperateStatementOrderDetailDOMap.put(oneStatementOrderDetailDO.getId(), oneStatementOrderDetailDO);
                    }
                }
                //转移结算单项
                if (CollectionUtil.isNotEmpty(needOperateStatementOrderDetailDOMap.keySet())) {
                    ...
                    BigDecimal needChangeStatementAmount = BigDecimal.ZERO; //结算单总金额
                    BigDecimal needChangeStatementRentDepositAmount = BigDecimal.ZERO;//结算租金押金金额
                    BigDecimal needChangeStatementDepositAmount = BigDecimal.ZERO;//结算押金金额
                    BigDecimal needChangeStatementRentAmount = BigDecimal.ZERO;//结算租金金额
                    BigDecimal needChangeStatementOtherAmount = BigDecimal.ZERO;//其他费用
                    BigDecimal needChangeStatementOverdueAmount = BigDecimal.ZERO;//结算单逾期金额
                    BigDecimal needChangeStatementCorrectAmount = BigDecimal.ZERO;//结算单冲正金额
                    BigDecimal needChangeStatementRentDepositPaidAmount = BigDecimal.ZERO;//已支付结算租金押金金额
                    BigDecimal needChangeStatementDepositPaidAmount = BigDecimal.ZERO;//已支付结算押金金额
                    BigDecimal needChangeStatementRentPaidAmount = BigDecimal.ZERO;//已支付结算租金金额
                    BigDecimal needChangeStatementOtherPaidAmount = BigDecimal.ZERO;//已支付其他费用
                    BigDecimal needChangeStatementOverduePaidAmount = BigDecimal.ZERO;//已支付结算单逾期金额
                    BigDecimal needChangeStatementDepositReturnAmount = BigDecimal.ZERO;//退还押金金额
                    BigDecimal needChangeStatementRentDepositReturnAmount = BigDecimal.ZERO;//退还租金押金金额
                    BigDecimal needChangeStatementCouponAmount = BigDecimal.ZERO;//结算单优惠券
                    BigDecimal needChangeStatementPenaltyAmount = BigDecimal.ZERO;//结算单违约金
                    BigDecimal needChangeStatementPenaltyPaidAmount = BigDecimal.ZERO;//已支付结算单违约金

                    Integer customerId = statementOrderDetailDODB.getCustomerId();
                    Date dateKey = com.lxzl.se.common.util.date.DateUtil.getBeginOfDay(statementExpectPayTime);
                    StatementOrderDO statementOrderDO = statementOrderMapper.findByCustomerAndPayTime(customerId, dateKey);
                    if (statementOrderDO != null) {
                        statementOrderDOMap.put(dateKey, statementOrderDO);
                    }

                    for (Integer statementOrderDOId : needOperateStatementOrderDetailDOMap.keySet()) {
                        StatementOrderDetailDO statementOrderDetailDO = needOperateStatementOrderDetailDOMap.get(statementOrderDOId);
                        if (statementOrderDetailDO == null) {
                            continue;
                        }
                        ...
                        if (!statementOrderDOMap.containsKey(dateKey)) {
                            ...
                            statementOrderDO.setCreateTime(currentTime);
                            statementOrderDO.setUpdateTime(currentTime);
                            statementOrderDO.setStatementOverdueAmount(statementOrderDetailDO.getStatementDetailOverdueAmount());
                            statementOrderDO.setStatementCorrectAmount(statementOrderDetailDO.getStatementDetailCorrectAmount());
                            statementOrderDO.setStatementRentDepositPaidAmount(statementOrderDetailDO.getStatementDetailRentDepositPaidAmount());
                            statementOrderDO.setStatementDepositPaidAmount(statementOrderDetailDO.getStatementDetailDepositPaidAmount());
                            statementOrderDO.setStatementRentPaidAmount(statementOrderDetailDO.getStatementDetailRentPaidAmount());
                            statementOrderDO.setStatementOtherPaidAmount(statementOrderDetailDO.getStatementDetailOtherPaidAmount());
                            statementOrderDO.setStatementOverduePaidAmount(statementOrderDetailDO.getStatementDetailOverduePaidAmount());
                            statementOrderDO.setStatementDepositReturnAmount(statementOrderDetailDO.getStatementDetailDepositReturnAmount());
                            statementOrderDO.setStatementRentDepositReturnAmount(statementOrderDetailDO.getStatementDetailRentDepositReturnAmount());
                            statementOrderDO.setStatementCouponAmount(statementOrderDetailDO.getStatementCouponAmount());
                            statementOrderDO.setStatementPenaltyAmount(statementOrderDetailDO.getStatementDetailPenaltyAmount());
                            statementOrderDO.setStatementPenaltyPaidAmount(statementOrderDetailDO.getStatementDetailPenaltyPaidAmount());
                            statementOrderMapper.save(statementOrderDO);
                        } else {
                            statementOrderDO = statementOrderDOMap.get(dateKey);
                            statementOrderDO.setStatementAmount(BigDecimalUtil.add(BigDecimalUtil.round(statementOrderDO.getStatementAmount(), BigDecimalUtil.STANDARD_SCALE), BigDecimalUtil.round(statementOrderDetailDO.getStatementDetailAmount(), BigDecimalUtil.STANDARD_SCALE)));
                            ...
                            statementOrderDO.setStatementOverdueAmount(BigDecimalUtil.add(statementOrderDO.getStatementOverdueAmount(), statementOrderDetailDO.getStatementDetailOverdueAmount()));
                            statementOrderDO.setStatementCorrectAmount(BigDecimalUtil.add(statementOrderDO.getStatementCorrectAmount(), statementOrderDetailDO.getStatementDetailCorrectAmount()));
                            statementOrderDO.setStatementRentDepositPaidAmount(BigDecimalUtil.add(statementOrderDO.getStatementRentDepositPaidAmount(), statementOrderDetailDO.getStatementDetailRentDepositPaidAmount()));
                            statementOrderDO.setStatementDepositPaidAmount(BigDecimalUtil.add(statementOrderDO.getStatementDepositPaidAmount(), statementOrderDetailDO.getStatementDetailDepositPaidAmount()));
                            statementOrderDO.setStatementRentPaidAmount(BigDecimalUtil.add(statementOrderDO.getStatementRentPaidAmount(), statementOrderDetailDO.getStatementDetailRentPaidAmount()));
                            statementOrderDO.setStatementOtherPaidAmount(BigDecimalUtil.add(statementOrderDO.getStatementOtherPaidAmount(), statementOrderDetailDO.getStatementDetailOtherPaidAmount()));
                            statementOrderDO.setStatementOverduePaidAmount(BigDecimalUtil.add(statementOrderDO.getStatementOverduePaidAmount(), statementOrderDetailDO.getStatementDetailOverduePaidAmount()));
                            statementOrderDO.setStatementDepositReturnAmount(BigDecimalUtil.add(statementOrderDO.getStatementDepositReturnAmount(), statementOrderDetailDO.getStatementDetailDepositReturnAmount()));
                            statementOrderDO.setStatementRentDepositReturnAmount(BigDecimalUtil.add(statementOrderDO.getStatementRentDepositReturnAmount(), statementOrderDetailDO.getStatementDetailRentDepositReturnAmount()));
                            statementOrderDO.setStatementPenaltyAmount(BigDecimalUtil.add(statementOrderDO.getStatementPenaltyAmount(), statementOrderDetailDO.getStatementDetailPenaltyAmount()));
                            statementOrderDO.setStatementPenaltyPaidAmount(BigDecimalUtil.add(statementOrderDO.getStatementPenaltyPaidAmount(), statementOrderDetailDO.getStatementDetailPenaltyPaidAmount()));
                            statementOrderDO.setStatementCouponAmount(BigDecimalUtil.add(statementOrderDO.getStatementCouponAmount(), statementOrderDetailDO.getStatementCouponAmount()));
                            statementOrderMapper.update(statementOrderDO);
                        }
                        statementOrderDOMap.put(dateKey, statementOrderDO);
                        statementOrderDetailDO.setStatementOrderId(statementOrderDO.getId());

                        needOperateStatementOrderDetailDOList.add(statementOrderDetailDO);//用于更新结算详情的 结算单id和支付时间
                        //记录结算单项的各个金额
                        needChangeStatementAmount = BigDecimalUtil.add(needChangeStatementAmount, statementOrderDetailDO.getStatementDetailAmount());
                        needChangeStatementRentDepositAmount = BigDecimalUtil.add(needChangeStatementRentDepositAmount, statementOrderDetailDO.getStatementDetailRentDepositAmount());
                        needChangeStatementDepositAmount = BigDecimalUtil.add(needChangeStatementDepositAmount, statementOrderDetailDO.getStatementDetailDepositAmount());
                        needChangeStatementRentAmount = BigDecimalUtil.add(needChangeStatementRentAmount, statementOrderDetailDO.getStatementDetailRentAmount());
                        needChangeStatementOtherAmount = BigDecimalUtil.add(needChangeStatementOtherAmount, statementOrderDetailDO.getStatementDetailOtherAmount());
                        needChangeStatementOverdueAmount = BigDecimalUtil.add(needChangeStatementOverdueAmount, statementOrderDetailDO.getStatementDetailOverdueAmount());
                        needChangeStatementCorrectAmount = BigDecimalUtil.add(needChangeStatementCorrectAmount, statementOrderDetailDO.getStatementDetailCorrectAmount());
                        needChangeStatementRentDepositPaidAmount = BigDecimalUtil.add(needChangeStatementRentDepositPaidAmount, statementOrderDetailDO.getStatementDetailRentDepositPaidAmount());
                        needChangeStatementDepositPaidAmount = BigDecimalUtil.add(needChangeStatementDepositPaidAmount, statementOrderDetailDO.getStatementDetailDepositPaidAmount());
                        needChangeStatementRentPaidAmount = BigDecimalUtil.add(needChangeStatementRentPaidAmount, statementOrderDetailDO.getStatementDetailRentPaidAmount());
                        needChangeStatementOtherPaidAmount = BigDecimalUtil.add(needChangeStatementOtherPaidAmount, statementOrderDetailDO.getStatementDetailOtherPaidAmount());
                        needChangeStatementOverduePaidAmount = BigDecimalUtil.add(needChangeStatementOverduePaidAmount, statementOrderDetailDO.getStatementDetailOverduePaidAmount());
                        needChangeStatementDepositReturnAmount = BigDecimalUtil.add(needChangeStatementDepositReturnAmount, statementOrderDetailDO.getStatementDetailDepositReturnAmount());
                        needChangeStatementRentDepositReturnAmount = BigDecimalUtil.add(needChangeStatementRentDepositReturnAmount, statementOrderDetailDO.getStatementDetailRentDepositReturnAmount());
                        needChangeStatementCouponAmount = BigDecimalUtil.add(needChangeStatementCouponAmount, statementOrderDetailDO.getStatementCouponAmount());
                        needChangeStatementPenaltyAmount = BigDecimalUtil.add(needChangeStatementPenaltyAmount, statementOrderDetailDO.getStatementDetailPenaltyAmount());
                        needChangeStatementPenaltyPaidAmount = BigDecimalUtil.add(needChangeStatementPenaltyPaidAmount, statementOrderDetailDO.getStatementDetailPenaltyPaidAmount());
                    }

                    //更新转移后的结算单项 结算单id和支付时间
                    if (CollectionUtil.isNotEmpty(needOperateStatementOrderDetailDOList)) {
                        statementOrderDetailMapper.batchUpdate(needOperateStatementOrderDetailDOList);
                    }

                    StatementOrderDO needOperateStatementOrderDO = statementOrderMapper.findById(statementOrderDetailDODB.getStatementOrderId());
                    //结算单项转移后，需更新原结算单金额及状态
                    if ((statementExpectPayTime != null)
                            && (statementExpectPayTime.compareTo(needOperateStatementOrderDO.getStatementExpectPayTime()) != 0)){
                        needOperateStatementOrderDO.setStatementExpectPayTime(statementExpectPayTime);
                        needOperateStatementOrderDO.setStatementAmount(BigDecimalUtil.sub(needOperateStatementOrderDO.getStatementAmount(), needChangeStatementAmount));
                        needOperateStatementOrderDO.setStatementRentDepositAmount(BigDecimalUtil.sub(needOperateStatementOrderDO.getStatementRentDepositAmount(), needChangeStatementRentDepositAmount));
                        needOperateStatementOrderDO.setStatementDepositAmount(BigDecimalUtil.sub(needOperateStatementOrderDO.getStatementDepositAmount(), needChangeStatementDepositAmount));
                        needOperateStatementOrderDO.setStatementRentAmount(BigDecimalUtil.sub(needOperateStatementOrderDO.getStatementRentAmount(), needChangeStatementRentAmount));
                        needOperateStatementOrderDO.setStatementOtherAmount(BigDecimalUtil.sub(needOperateStatementOrderDO.getStatementOtherAmount(), needChangeStatementOtherAmount));
                        needOperateStatementOrderDO.setStatementOverdueAmount(BigDecimalUtil.sub(needOperateStatementOrderDO.getStatementOverdueAmount(), needChangeStatementOverdueAmount));
                        needOperateStatementOrderDO.setStatementCorrectAmount(BigDecimalUtil.sub(needOperateStatementOrderDO.getStatementCorrectAmount(), needChangeStatementCorrectAmount));
                        needOperateStatementOrderDO.setStatementRentDepositPaidAmount(BigDecimalUtil.sub(needOperateStatementOrderDO.getStatementRentDepositPaidAmount(), needChangeStatementRentDepositPaidAmount));
                        needOperateStatementOrderDO.setStatementDepositPaidAmount(BigDecimalUtil.sub(needOperateStatementOrderDO.getStatementDepositPaidAmount(), needChangeStatementDepositPaidAmount));
                        needOperateStatementOrderDO.setStatementRentPaidAmount(BigDecimalUtil.sub(needOperateStatementOrderDO.getStatementRentPaidAmount(), needChangeStatementRentPaidAmount));
                        needOperateStatementOrderDO.setStatementOtherPaidAmount(BigDecimalUtil.sub(needOperateStatementOrderDO.getStatementOtherPaidAmount(), needChangeStatementOtherPaidAmount));
                        needOperateStatementOrderDO.setStatementOverduePaidAmount(BigDecimalUtil.sub(needOperateStatementOrderDO.getStatementOverduePaidAmount(), needChangeStatementOverduePaidAmount));
                        needOperateStatementOrderDO.setStatementDepositReturnAmount(BigDecimalUtil.sub(needOperateStatementOrderDO.getStatementDepositReturnAmount(), needChangeStatementDepositReturnAmount));
                        needOperateStatementOrderDO.setStatementRentDepositReturnAmount(BigDecimalUtil.sub(needOperateStatementOrderDO.getStatementRentDepositReturnAmount(), needChangeStatementRentDepositReturnAmount));
                        needOperateStatementOrderDO.setStatementCouponAmount(BigDecimalUtil.sub(needOperateStatementOrderDO.getStatementCouponAmount(), needChangeStatementCouponAmount));
                        needOperateStatementOrderDO.setStatementPenaltyAmount(BigDecimalUtil.sub(needOperateStatementOrderDO.getStatementPenaltyAmount(), needChangeStatementPenaltyAmount));
                        needOperateStatementOrderDO.setStatementPenaltyPaidAmount(BigDecimalUtil.sub(needOperateStatementOrderDO.getStatementPenaltyPaidAmount(), needChangeStatementPenaltyPaidAmount));

                        needOperateStatementOrderDO.setUpdateUser(loginUserId.toString());
                        needOperateStatementOrderDO.setUpdateTime(currentTime);
                        statementOrderMapper.update(needOperateStatementOrderDO);
                    }

                    //更新原结算单状态等
                    statementOrderSupport.fixOneStatementOrderStatementTime(needOperateStatementOrderDO);
    ...
    private Integer getStatementOrderStatusByDetail(Integer statementOrderId){
        ...
        boolean hasNotPaidDetail = false;
        boolean hasCorrectDetail = false;
        boolean hasNotNeedPaidDetail = false;
        List<StatementOrderDetailDO> statementOrderDetailDOListDBNew = statementOrderDetailMapper.findByStatementOrderId(statementOrderId);
        for (StatementOrderDetailDO oneStatementOrderDetailDO : statementOrderDetailDOListDBNew){
            if (StatementOrderStatus.STATEMENT_ORDER_STATUS_INIT.equals(oneStatementOrderDetailDO.getStatementDetailStatus())){
                hasNotPaidDetail = true;
            }
            else if (StatementOrderStatus.STATEMENT_ORDER_STATUS_SETTLED.equals(oneStatementOrderDetailDO.getStatementDetailStatus())){
                hasPaidDetail = true;
            }
            else if (StatementOrderStatus.STATEMENT_ORDER_STATUS_NO.equals(oneStatementOrderDetailDO.getStatementDetailStatus())){
                hasNotNeedPaidDetail = true;
            }
            else if (StatementOrderStatus.STATEMENT_ORDER_STATUS_CORRECTED.equals(oneStatementOrderDetailDO.getStatementDetailStatus())){
                hasCorrectDetail = true;
            }
        }
        if (hasPaidDetail && !hasNotPaidDetail){//已支付
            return StatementOrderStatus.STATEMENT_ORDER_STATUS_SETTLED;
        }
        else if (!hasPaidDetail && hasNotPaidDetail){ //未支付
            return StatementOrderStatus.STATEMENT_ORDER_STATUS_INIT;
        }
        else if (hasPaidDetail && hasNotPaidDetail){ //部分支付
            return StatementOrderStatus.STATEMENT_ORDER_STATUS_SETTLED_PART;
        }
        else if (!hasPaidDetail && !hasNotPaidDetail && hasCorrectDetail){//当明细没有已支付和未支付时 有已冲正则返回已冲正
            return StatementOrderStatus.STATEMENT_ORDER_STATUS_CORRECTED;
        }
        else {
            return StatementOrderStatus.STATEMENT_ORDER_STATUS_NO;
        }
    }
...
//            Date thisPhaseEndTime = thisPhaseDate;
    ...
    StatementOrderDetailDO calculateOneReletStatementOrderDetail(Integer rentType, Integer rentTimeLength, Integer paymentCycle, Integer payMode, Date rentStartTime, BigDecimal statementDetailAmount, Integer customerId, Integer orderId, Integer orderItemType, Integer orderItemReferId, Date currentTime, Integer loginUserId, Integer reletOrderItemReferId) {
        ...
        return statementCommonSupport.buildStatementOrderDetailDO(customerId, OrderType.ORDER_TYPE_ORDER, orderId, orderItemType, orderItemReferId, statementExpectPayTime, rentStartTime, statementEndTime, statementDetailAmount, BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO, currentTime, loginUserId, reletOrderItemReferId);
    }
...
//            Date thisPhaseEndTime = thisPhaseDate;
        ...
        if (OrderPayMode.PAY_MODE_PAY_BEFORE.equals(payMode)) {
            statementExpectPayTime = statementStartTime;
        ...
        return statementCommonSupport.buildStatementOrderDetailDO(customerId, OrderType.ORDER_TYPE_ORDER, orderId, orderItemType, orderItemReferId, statementExpectPayTime, rentStartTime, statementEndTime, firstPhaseAmount, BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO, currentTime, loginUserId, reletOrderItemReferId);
    }
    ...
    StatementOrderDetailDO calculateLastStatementOrderDetail(Integer customerId, Integer orderId, Integer orderItemType, Integer orderItemReferId, Date lastCalculateDate, Date rentStartTime, Integer payMode, Integer rentType, Integer rentTimeLength, BigDecimal itemAllAmount, BigDecimal alreadyPaidAmount, Date currentTime, Integer loginUserId, Integer reletOrderItemReferId) {
        ...
        return statementCommonSupport.buildStatementOrderDetailDO(customerId, OrderType.ORDER_TYPE_ORDER, orderId, orderItemType, orderItemReferId, statementExpectPayTime, lastPhaseStartTime, statementEndTime, lastPhaseAmount, BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO, currentTime, loginUserId, reletOrderItemReferId);
    }
    ...
    StatementOrderDetailDO calculateMiddleStatementOrderDetail(Integer rentType, Integer customerId, Integer orderId, Integer orderItemType, Integer orderItemReferId, Date lastCalculateDate, Integer paymentCycle, BigDecimal itemUnitAmount, Integer itemCount, Integer statementDays, Integer payMode, Date currentTime, Integer loginUserId, Integer reletOrderItemReferId) {
        ...
        return statementCommonSupport.buildStatementOrderDetailDO(customerId, OrderType.ORDER_TYPE_ORDER, orderId, orderItemType, orderItemReferId, statementExpectPayTime, thisPhaseStartTime, statementEndTime, middlePhaseAmount, BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO, currentTime, loginUserId, reletOrderItemReferId);
    }
...
//                    statementOrderDO.setStatementStatus(StatementOrderStatus.STATEMENT_ORDER_STATUS_NO);
    ...
    ServiceResult<String, AmountNeedReturn> clearStatementOrder(OrderDO orderDO, boolean clearStatementDateSplitCfg) {
        ...
        //清除结算信息
        ServiceResult<String, AmountNeedReturn> result = statementOrderSupport.clearStatement(paid, orderDO.getBuyerCustomerNo(), statementOrderDetailDOList);
        //创建失败回滚
        if (!ErrorCode.SUCCESS.equals(result.getErrorCode())) {
            TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();//回滚
            return result;
    ...
    private List<StatementOrderDetailDO> generateReletStatementDetailList(ReletOrderDO reletOrderDO, Date currentTime, Integer statementDays, Integer loginUserId, boolean isNeedAlign) {
        return generateReletStatementDetailList(reletOrderDO, currentTime, statementDays, loginUserId, isNeedAlign, CommonConstant.PROPORTION_MAX);
    }

    private List<StatementOrderDetailDO> generateReletStatementDetailList(ReletOrderDO reletOrderDO, Date currentTime, Integer statementDays, Integer loginUserId, boolean isNeedAlign, double amountPercent) {
        List<StatementOrderDetailDO> addStatementOrderDetailDOList = new ArrayList<>();
        Date rentStartTime = reletOrderDO.getRentStartTime();
...
//                }
    ...
    ServiceResult<String, AmountNeedReturn> clearReletOrderStatement(ReletOrderDO reletOrderDO) {
        ...
        boolean paid = true;
        //清除结算信息
        ServiceResult<String, AmountNeedReturn> result = statementOrderSupport.clearStatement(paid, reletOrderDO.getBuyerCustomerNo(), statementOrderDetailDOList);
        //创建失败回滚
        if (!ErrorCode.SUCCESS.equals(result.getErrorCode())) {
            TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();//回滚
            return result;
...
//                BigDecimal thisReturnRentDepositAmount = BigDecimalUtil.mul(BigDecimalUtil.div(orderProductDO.getRentDepositAmount(), new BigDecimal(orderProductDO.getProductCount()), BigDecimalUtil.SCALE), returnCount);
//                BigDecimal thisReturnDepositAmount = BigDecimalUtil.mul(BigDecimalUtil.div(orderProductDO.getDepositAmount(), new BigDecimal(orderProductDO.getProductCount()), BigDecimalUtil.SCALE), returnCount);
//                reStatementOrderSubDeposit(loginUserId, returnTime, currentTime, statementOrderDetailDO, thisReturnRentDepositAmount, thisReturnDepositAmount,k3ReturnOrderDetailDO);
                deleteDepositReturnRecord(k3ReturnOrderDetailDO);
                continue;
            }
            statementDetailStartTime = statementOrderDetailDO.getStatementStartTime();
            statementDetailEndTime = statementOrderDetailDO.getStatementEndTime();
...
//                                payReturnAmount = statementOrderDetailDO.getStatementDetailAmount();
    ...
    /**
     * 重算时减去退货的押金（临时解决方案） 改为 重算押金以订单原额重新续交
     *
     * @param loginUserId
     * @param returnTime
     * @param currentTime
...
//        statementOrderMapper.update(statementOrderDO);
 ...
 //   }
     ...
     * @param k3ReturnOrderDetailDO
     */
    private void deleteDepositReturnRecord(K3ReturnOrderDetailDO k3ReturnOrderDetailDO){
        //删除退押金记录
        List<StatementOrderReturnDetailDO> returnDetailDOS=statementOrderReturnDetailMapper.findByReturnOrderDetailId(k3ReturnOrderDetailDO.getId(),OrderType.ORDER_TYPE_RETURN);
        if(CollectionUtil.isNotEmpty(returnDetailDOS)){
            statementOrderReturnDetailMapper.batchDelete(returnDetailDOS);
        }
    }
                    ...
                    if (OrderType.ORDER_TYPE_ORDER.equals(statementOrderDetail.getOrderType())) {
                        ...
                        if (OrderItemType.ORDER_ITEM_TYPE_PRODUCT.equals(statementOrderDetail.getOrderItemType())) {
                            ...
                            if (orderProductDO != null) {
                                Integer productId = orderProductDO.getProductId();
                                Integer isNewProduct = orderProductDO.getIsNewProduct();
                                key = statementOrderDetail.getOrderItemReferId() + statementOrderDetail.getOrderItemType() + "-" + statementOrderDetail.getOrderType() + "-" + productId + "-" + isNewProduct + "-" + statementOrderDetail.getOrderNo() + "-" + statementOrderDetail.getItemRentType() + "-" + statementOrderDetail.getReletOrderItemReferId() + "-" + statementOrderDetail.getStatementExpectPayEndTime() + "-" + statementOrderDetail.getStatementExpectPayTime();
                        ...
                        if (OrderItemType.ORDER_ITEM_TYPE_MATERIAL.equals(statementOrderDetail.getOrderItemType())) {
                            ...
                            if (orderMaterialDO != null) {
                                Integer materialId = orderMaterialDO.getMaterialId();
                                Integer isNewMaterial = orderMaterialDO.getIsNewMaterial();
                                key = statementOrderDetail.getOrderItemReferId() + statementOrderDetail.getOrderItemType() + "-" + statementOrderDetail.getOrderType() + "-" + materialId + "-" + isNewMaterial + "-" + statementOrderDetail.getOrderNo() + "-" + statementOrderDetail.getItemRentType() + "-" + statementOrderDetail.getReletOrderItemReferId() + "-" + statementOrderDetail.getStatementExpectPayEndTime() + "-" + statementOrderDetail.getStatementExpectPayTime();
                        ...
                        //为订单其他时
                        if (OrderItemType.ORDER_ITEM_TYPE_OTHER.equals(statementOrderDetail.getOrderItemType())) {
                            key = statementOrderDetail.getOrderItemReferId() + statementOrderDetail.getOrderItemType() + "-" + statementOrderDetail.getOrderType() + "-" + statementOrderDetail.getOrderNo() + "-" + statementOrderDetail.getItemRentType() + "-" + statementOrderDetail.getReletOrderItemReferId() + "-" + statementOrderDetail.getStatementExpectPayEndTime() + "-" + statementOrderDetail.getStatementExpectPayTime();
                            hashMap.put(key, statementOrderDetail);
                            continue;
                        }
                    }
...
//                                }
                        ...
                        //为退还物料时
                        if (OrderItemType.ORDER_ITEM_TYPE_RETURN_OTHER.equals(statementOrderDetail.getOrderItemType())) {
                            key = statementOrderDetail.getOrderItemType() + "-" + statementOrderDetail.getOrderType() + "-" + statementOrderDetail.getOrderNo() + "-" + statementOrderDetail.getItemRentType() + "-" + statementOrderDetail.getOrderItemReferId()+ "-" + statementOrderDetail.getReletOrderItemReferId() + "-" + statementOrderDetail.getStatementExpectPayEndTime() + "-" + statementOrderDetail.getStatementExpectPayTime();
                        }
...
//                            statementOrderDetail.setItemRentType(reletOrderDO.getRentType());
    ...
    public ServiceResult<String, Boolean> payStatementOrderDetail(List<Integer> mergeStatementItemList) {
        ...
        for(StatementOrderDetailDO statementOrderDetailDO:statementOrderDetailDOList){
            ...
            AmountNeedPay amountNeedPay=statementPaySupport.calculateStatementItemRealNeedPay(statementOrderDetailDO);
            String loginUserId=userSupport.getCurrentUserId().toString();

            //更新结算单项
            if(BigDecimalUtil.compare(amountNeedPay.getTotalAmount(),BigDecimal.ZERO)>0){
                statementOrderDetailDO.setStatementDetailStatus(StatementOrderStatus.STATEMENT_ORDER_STATUS_SETTLED);
                statementOrderDetailDO.setStatementDetailPaidTime(currentTime);
                statementOrderDetailDO.setUpdateTime(currentTime);
                ...
                statementOrderDetailMapper.update(statementOrderDetailDO);
                //修改结算单项关联订单或续租单
                updateOrderAfterStatementItemPay(statementOrderDetailDO, amountNeedPay.getRentAmount(), currentTime, loginUserId);

                rentPayAmount=BigDecimalUtil.add(rentPayAmount,amountNeedPay.getRentAmount());
                rentDepositPayAmount=BigDecimalUtil.add(rentDepositPayAmount,amountNeedPay.getRentDepositAmount());
                depositPayAmount=BigDecimalUtil.add(depositPayAmount,amountNeedPay.getDepositAmount());
                otherPayAmount=BigDecimalUtil.add(otherPayAmount,amountNeedPay.getOtherAmount());
                overduePayAmount=BigDecimalUtil.add(overduePayAmount,amountNeedPay.getOverdueAmount());
                realTotalNeedPayAmount=BigDecimalUtil.add(realTotalNeedPayAmount,amountNeedPay.getTotalAmount());
            }
        }
        //根据结算单项支付金额修改结算单
    ...
    @Override
    public ServiceResult<String, BigDecimal> queryStatementOrderDetailsNeedPay(List<Integer> mergeStatementItemList) {
        ServiceResult<String, BigDecimal> result = new ServiceResult<>();
        if(CollectionUtil.isEmpty(mergeStatementItemList)){
            result.setErrorCode(ErrorCode.PARAM_IS_ERROR);
            return result;
        ...
        BigDecimal  realTotalNeedPayAmount=BigDecimal.ZERO;
        for(StatementOrderDetailDO statementOrderDetailDO:statementOrderDetailDOList){
            if(OrderType.ORDER_TYPE_RETURN.equals(statementOrderDetailDO.getOrderType())&&statementOrderDetailDO.getReturnReferId()!=null)continue;
            if (StatementOrderStatus.STATEMENT_ORDER_STATUS_SETTLED.equals(statementOrderDetailDO.getStatementDetailStatus())
                    || StatementOrderStatus.STATEMENT_ORDER_STATUS_NO.equals(statementOrderDetailDO.getStatementDetailStatus())) {
        ...
        //支付
        result.setResult(realTotalNeedPayAmount);
        result.setErrorCode(ErrorCode.SUCCESS);
        return result;
    }
    ...
    /**
     * @desciption: 退货单反向流程，重置退货单相关操作
     * @param returnOrderNo
     * @return
     */
    @Override
    @Transactional(readOnly = false, isolation = Isolation.REPEATABLE_READ, propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
    public ServiceResult<String,String> rollbackSuccessReturnOrder(String returnOrderNo){
        ServiceResult<String, String> result = new ServiceResult<>();
        K3ReturnOrderDO k3ReturnOrderDO = k3ReturnOrderMapper.findByNo(returnOrderNo);
        if (k3ReturnOrderDO == null) {
            result.setErrorCode(ErrorCode.K3_RETURN_ORDER_IS_NOT_NULL);
        ...
        //处理退过的押金(必须放在最后)
        String resultCode= autoRepayDepositRollReturn(k3ReturnOrderDO);
        if(!ErrorCode.SUCCESS.equals(resultCode)){
            result.setErrorCode(resultCode);
            TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
            return result;
    ...
    /**
     * repay the deposit amount and return returnOrder's other fee,when roll back return order
     * add renting count
     * update return order
     * @param k3ReturnOrderDO
     */
    private String autoRepayDepositRollReturn(K3ReturnOrderDO k3ReturnOrderDO){
        ...
        List<StatementOrderDetailDO> needUpdateDetail=new ArrayList<>();
        Map<Integer,StatementOrderDO> statementOrderCatch=new HashMap<>();
        BigDecimal needRepayDeposit=BigDecimal.ZERO;
        BigDecimal needRepayRentDeposit=BigDecimal.ZERO;
        List<OrderProductDO> needUpdateOrderProductList=new ArrayList<>();
        List<OrderMaterialDO> needUpdateOrderMaterialList=new ArrayList<>();

        Date currentTime=new Date();
        String userId=userSupport.getCurrentUserId().toString();
        List<StatementOrderReturnDetailDO> depositReturnRecordList=statementOrderReturnDetailMapper.findByReturnOrderId(k3ReturnOrderDO.getId(),OrderType.ORDER_TYPE_RETURN);
        ...
        for(K3ReturnOrderDetailDO k3ReturnOrderDetailDO:k3ReturnOrderDetailDOList){
            ...
            if (productSupport.isProduct(k3ReturnOrderDetailDO.getProductNo())) {
                OrderProductDO orderProductDO = productSupport.getOrderProductDO(k3ReturnOrderDetailDO.getOrderNo(), k3ReturnOrderDetailDO.getOrderItemId(), k3ReturnOrderDetailDO.getOrderEntry());
                if (orderProductDO == null) continue;
                orderProductDO.setRentingProductCount(orderProductDO.getRentingProductCount()+returnCount.intValue());
            ...
            if(productSupport.isMaterial(k3ReturnOrderDetailDO.getProductNo())){
                OrderMaterialDO orderMaterialDO = productSupport.getOrderMaterialDO(k3ReturnOrderDetailDO.getOrderNo(), k3ReturnOrderDetailDO.getOrderItemId(), k3ReturnOrderDetailDO.getOrderEntry());
                if(orderMaterialDO==null)continue;
                orderMaterialDO.setRentingMaterialCount(orderMaterialDO.getRentingMaterialCount()+returnCount.intValue());
            ...
            for(StatementOrderDetailDO statementOrderDetailDO:depositList){
                ...
                if(!statementOrderCatch.containsKey(statementOrderId)){
                    StatementOrderDO statementOrderDO = statementOrderMapper.findById(statementOrderId);
                    statementOrderCatch.put(statementOrderId,statementOrderDO);
                }
                StatementOrderDO statementOrderDO=statementOrderCatch.get(statementOrderId);
                for (StatementOrderReturnDetailDO returnRecord:returnRecords){
                    if(StatementReturnType.RETURN_TYPE_DEPOSIT.equals(returnRecord.getReturnType())){
                        BigDecimal thisReturnDepositAmount=returnRecord.getReturnAmount();
                        if(BigDecimalUtil.compare(statementOrderDetailDO.getStatementDetailDepositReturnAmount(),thisReturnDepositAmount)>=0){
                            statementOrderDetailDO.setStatementDetailDepositReturnAmount(BigDecimalUtil.sub(statementOrderDetailDO.getStatementDetailDepositReturnAmount(), thisReturnDepositAmount));
                            statementOrderDO.setStatementDepositReturnAmount(BigDecimalUtil.sub(statementOrderDO.getStatementDepositReturnAmount(), thisReturnDepositAmount));
                        } else{
                            statementOrderDetailDO.setStatementDetailDepositAmount(BigDecimalUtil.add(statementOrderDetailDO.getStatementDetailDepositAmount(),thisReturnDepositAmount));
                            statementOrderDetailDO.setStatementDetailAmount(BigDecimalUtil.add(statementOrderDetailDO.getStatementDetailAmount(),thisReturnDepositAmount));
                            statementOrderDetailDO.setStatementDetailDepositPaidAmount(BigDecimalUtil.add(statementOrderDetailDO.getStatementDetailDepositPaidAmount(),thisReturnDepositAmount));

                            statementOrderDO.setStatementDepositAmount(BigDecimalUtil.add(statementOrderDO.getStatementDepositAmount(),thisReturnDepositAmount));
                            statementOrderDO.setStatementAmount(BigDecimalUtil.add(statementOrderDO.getStatementAmount(),thisReturnDepositAmount));
                            statementOrderDO.setStatementDepositPaidAmount(BigDecimalUtil.add(statementOrderDO.getStatementDepositPaidAmount(),thisReturnDepositAmount));
                        }
                        needRepayDeposit=BigDecimalUtil.add(needRepayDeposit,thisReturnDepositAmount);
                    }
                    if(StatementReturnType.RETURN_TYPE_RENT_DEPOSIT.equals(returnRecord.getReturnType())){
                        BigDecimal thisReturnRentDepositAmount=returnRecord.getReturnAmount();
                        if(BigDecimalUtil.compare(statementOrderDetailDO.getStatementDetailRentDepositReturnAmount(),thisReturnRentDepositAmount)>=0){
                            statementOrderDetailDO.setStatementDetailRentDepositReturnAmount(BigDecimalUtil.sub(statementOrderDetailDO.getStatementDetailRentDepositReturnAmount(), thisReturnRentDepositAmount));
                            statementOrderDO.setStatementRentDepositReturnAmount(BigDecimalUtil.sub(statementOrderDO.getStatementRentDepositReturnAmount(), thisReturnRentDepositAmount));
                        } else{
                            statementOrderDetailDO.setStatementDetailRentDepositAmount(BigDecimalUtil.add(statementOrderDetailDO.getStatementDetailRentDepositAmount(),thisReturnRentDepositAmount));
                            statementOrderDetailDO.setStatementDetailAmount(BigDecimalUtil.add(statementOrderDetailDO.getStatementDetailAmount(),thisReturnRentDepositAmount));
                            statementOrderDetailDO.setStatementDetailRentDepositPaidAmount(BigDecimalUtil.add(statementOrderDetailDO.getStatementDetailRentDepositPaidAmount(),thisReturnRentDepositAmount));

                            statementOrderDO.setStatementRentDepositAmount(BigDecimalUtil.add(statementOrderDO.getStatementRentDepositAmount(),thisReturnRentDepositAmount));
                            statementOrderDO.setStatementAmount(BigDecimalUtil.add(statementOrderDO.getStatementAmount(),thisReturnRentDepositAmount));
                            statementOrderDO.setStatementRentDepositPaidAmount(BigDecimalUtil.add(statementOrderDO.getStatementRentDepositPaidAmount(),thisReturnRentDepositAmount));
                        }
                        needRepayRentDeposit=BigDecimalUtil.add(needRepayRentDeposit,thisReturnRentDepositAmount);
                    }
                    needUpdateDetail.add(statementOrderDetailDO);
                }
            }
        }
        //return other amount,delte return other amount statement detail
        BigDecimal needReturnOtherAmount=BigDecimal.ZERO;
        List<StatementOrderDetailDO> otherStatementList= statementOrderDetailMapper.findByOrderIdAndStatementDetailType(k3ReturnOrderDO.getId(),OrderType.ORDER_TYPE_RETURN,StatementDetailType.STATEMENT_DETAIL_TYPE_OTHER);
        if(CollectionUtil.isNotEmpty(otherStatementList)){
            for(StatementOrderDetailDO statementOrderDetailDO:otherStatementList){
                Integer statementOrderId=statementOrderDetailDO.getStatementOrderId();
                if(!statementOrderCatch.containsKey(statementOrderId)){
                    StatementOrderDO statementOrderDO = statementOrderMapper.findById(statementOrderId);
                    statementOrderCatch.put(statementOrderId,statementOrderDO);
                ...
                needReturnOtherAmount=BigDecimalUtil.add(statementOrderDetailDO.getStatementDetailOtherPaidAmount(),needReturnOtherAmount);
                StatementOrderDO statementOrderDO=statementOrderCatch.get(statementOrderId);
                statementOrderDO.setStatementOtherPaidAmount(BigDecimalUtil.sub(statementOrderDO.getStatementOtherPaidAmount(),statementOrderDetailDO.getStatementDetailOtherPaidAmount()));
                ...
                statementOrderDO.setStatementAmount(BigDecimalUtil.sub(statementOrderDO.getStatementAmount(),statementOrderDetailDO.getStatementDetailOtherAmount()));
                needUpdateDetail.add(statementOrderDetailDO);
            }
        }
        //update ref statement order detail
        if(CollectionUtil.isNotEmpty(needUpdateDetail)){
            statementOrderDetailMapper.batchUpdate(needUpdateDetail);
        }
        //update ref statement order
        if(statementOrderCatch.size()>0){
            for(StatementOrderDO statementOrderDO:statementOrderCatch.values()){
                statementOrderMapper.update(statementOrderDO);
            }
        }
        //remove all return deposit record
        if(CollectionUtil.isNotEmpty(depositReturnRecordList)){
            statementOrderReturnDetailMapper.batchDelete(depositReturnRecordList);
        }
        //update order item
        if(CollectionUtil.isNotEmpty(needUpdateOrderProductList)){
            orderProductMapper.batchUpdateRentingCount(needUpdateOrderProductList);
        }
